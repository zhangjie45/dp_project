<template>
    <div class="main">
        <div class="header">
            <div class="header-title" v-if="![28, 33, 32].includes(props.deviceIndex)">下收卷数据监测</div>
            <div class="header-title" v-else>下卷数据监测</div>
        </div>
        <div class="content">
            <div class="top">
                <div class="item" v-if="![28, 33, 32].includes(props.deviceIndex)">
                    <div class="item-num">{{ formatDisplayValue(props.rewindData.lowerWinderCurrentTension) }}</div>
                    <div class="item-txt">下收卷当前张力(N)</div>
                </div>
                <div class="item">
                    <div class="item-num">{{ formatDisplayValue(props.rewindData.lowerWinderTaper, '%') }}</div>
                    <!-- <div class="item-num" v-else>{{ formatDisplayValue(props.rewindData.bTaper) }}</div> -->
                    <div class="item-txt" v-if="![28, 33, 32].includes(props.deviceIndex)">下收卷锥度设定</div>
                    <div class="item-txt" v-else>下卷锥度</div>
                </div>
                <div class="item">
                    <div class="item-num" v-if="![28, 33, 32].includes(props.deviceIndex)">{{
                        formatDisplayValue(props.rewindData.lowerWinderMaterialWidth) }}</div>
                    <div class="item-num" v-else>{{ formatDisplayValue(props.rewindData.lowerWinderTensionSet) }}</div>
                    <div class="item-txt" v-if="![28, 33, 32].includes(props.deviceIndex)">B轴收卷宽度(mm)</div>
                    <div class="item-txt" v-else>下卷张力设定(N)</div>
                </div>
            </div>
            <div class="bottom">
...
const props = defineProps({
    ...
    rewindData: {
        type: Object,
        default: () => ({
            downRollingCurrentTension: null,
            downRollingTensionSetting: null,
            brollingWidth: null
        })
    },
    deviceIndex: {
...
const getChartOptions = () => {
    const chartData = validateChartData(props.downData)
