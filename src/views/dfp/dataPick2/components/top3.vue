<template>
    <div class="main">
        <div class="header">
            <div class="header-title">电晕总电流监测</div>
        </div>
        <div class="content">
            <div class="select">
...
// import { getElectrodeCurrent } from '@/api/detail'
...
import { electrodeDataJson } from '../data'
const getData = () => {
    chartData.value = electrodeDataJson
    nextTick(() => {
        updateChartWithOptions()
    })
    // getElectrodeCurrent({ machineCode: props.deviceIndex, viewName: selectedDevice.value === 'all' ? undefined : selectedDevice.value })
    //     .then((res) => {
    //         if (res.code === 200) {
